...
namespace VAECaves
{
    public class CompConditionalSpawner : ThingComp
    {
        private int ticksUntilSpawn;

        public CompProperties_ConditionalSpawner PropsSpawner => (CompProperties_ConditionalSpawner)props;
        ...
        public bool TryDoSpawn()
        {
            ...
            List<Pawn> spidersInMap = (from x in this.parent.Map.mapPawns.AllPawnsSpawned
                                       where x.kindDef == PawnKindDef.Named("VAECaves_AncientGiantSpider")
                                       select x).ToList();

            if (spidersInMap.Count()==0)
            {
                return false;
            }
